...
const isInstallationWritable = require('../../utils/isInstallationWritable');
...
const autoUpdatePropertyName = 'autoUpdate';
...
class Config {
  constructor(serverless, options) {
    ...
    this.commands = {
      config: {
        ...
        options: {
          'autoupdate': {
            usage: 'Enable Serverless auto update',
          },
          'disable-autoupdate': {
            usage: 'Disable Serverless auto update',
          },
        },
      },
    };
  ...
  config() {
    if (this.options.autoupdate) {
      if (isInstallationWritable()) {
        configUtils.set(autoUpdatePropertyName, true);
      } else {
        throw new this.serverless.classes.Error(
          'Serverless installation seens to not be writable. Auto update cannot be enable.'
        );
      }
    } else if (this.options['disable-autoupdate']) {
      configUtils.set(autoUpdatePropertyName, false);
    }
  }
